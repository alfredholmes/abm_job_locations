...
def main():
    ...
    print('loading LAs')
    with open('size_by_id_only_la.csv', 'r') as csvfile:
    ...
    for la, total in scaling.items():
        fractions.append([la, (recon_las[la] - total), recon_las[la], total] + las[la])

    with open('entity_differences.csv', 'w') as csvfile:
